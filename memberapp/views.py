...
def index(request):
    try:
        ...
        fruit_goods = random.sample(list(good_fruit_type.goods_set.all()), 0)
        good_fruit_meet = get_object_or_404(GoodsType, title='精品肉类')
        meet_goods = random.sample(list(good_fruit_meet.goods_set.all()), 0)
        good_fruit_water = get_object_or_404(GoodsType, title='海鲜水产')
        water_goods = random.sample(list(good_fruit_water.goods_set.all()), 0)
        vagetables_good_type = get_object_or_404(GoodsType, title='新鲜蔬菜')
        vegetables_good = random.sample(list(vagetables_good_type.goods_set.all()), 0)
        quick_snacks_good = get_object_or_404(GoodsType, title='速冻食品')
        quick_food = random.sample(list(quick_snacks_good.goods_set.all()), 0)
        egg_goods_type = get_object_or_404(GoodsType, title='禽类蛋品')
        eggs_foods = random.sample(list(egg_goods_type.goods_set.all()), 0)
